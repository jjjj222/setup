...
Edge.prototype.getMaxY = function() {
  return d3.max(this.path, (n) => { return n.y; });
}

Edge.prototype.getMinY = function() {
  return d3.min(this.path, (n) => { return n.y; });
}

Edge.prototype.getMaxX = function() {
  return d3.max(this.path, (n) => { return n.x; });
}

Edge.prototype.getMinX = function() {
  return d3.min(this.path, (n) => { return n.x; });
}
//------------------------------------------------------------------------------
//   Graph
//------------------------------------------------------------------------------
...
Graph.prototype.getBBox = function() {
    ...
    let min_x = d3.min(this.nodes, (n) => { return n.pos.x; });
    let min_y = d3.min(this.nodes, (n) => { return n.pos.y; });
    let max_x = d3.max(this.nodes, (n) => { return n.pos.x + n.width; });
    let max_y = d3.max(this.nodes, (n) => { return n.pos.y + n.height; });

    if (this.edges.length != 0) {
      const edge_min_x = d3.min(this.edges, (e) => { return e.getMinX(); });
      const edge_min_y = d3.min(this.edges, (e) => { return e.getMinY(); });
      const edge_max_x = d3.max(this.edges, (e) => { return e.getMaxX(); });
      const edge_max_y = d3.max(this.edges, (e) => { return e.getMaxY(); });
