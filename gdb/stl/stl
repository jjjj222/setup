...
define dpvector
	...
	if $argc == 1
		...
		while $i < $size
			printf "elem[%u]: ", $i
			p (*($arg0._M_impl._M_start + $i)).dump_str()
			set $i++
		end
	end
	if $argc > 0
...
define pvector
    ...
    if $argc == 2
		...
		while $i < $size
			printf "elem[%u]: ", $i
            set $ptr = $arg0._M_impl._M_start + $i 
            if (*$ptr == 0)
    ...
    if $argc == 3
		...
		while $i < $size
			printf "elem[%u]: ", $i
			p (($arg2)(*($arg0._M_impl._M_start + $i)))$arg1
			set $i++
		end
    end
    # end add
	...
	if $argc > 0
        printf "Vector size = %u\n", $size
        #printf "Total: %u/%u, ", $size, $capacity
...
define plist
	if $argc == 0
		...
		while $current != $head
			...
			if $argc == 4
                printf "elem[%u]: ", $size
                p (($arg3)(*($arg1*)($current + 1)))$arg2
			end
			#if $argc == 5
			#	if $size == $arg5
            #        printf "elem[%u]: ", $size
            #        p (($arg4)(*($arg1*)($current + 1)))$arg3
			#	end
			#end
            # end add
